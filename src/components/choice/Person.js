...
const Person = (props) => {
    const [personClasses, setPersonClasses] = useState(classes["img-container"])
  const person = useRef();

  let imageSrc;
  let iconComponent;
  if (props.pictureName === "Cris") {
    imageSrc =
  ...
  const clickPgHandler = () => {
    ...
    setPersonClasses(`${classes.chosen} ${classes["img-container"]}`);
    let prevSibling = person.current.previousSibling
    if(prevSibling) prevSibling.classList.add('not-chosen')
    setTimeout(() => {
        setPersonClasses(classes["img-container"]);

        props.onChosen(person.current.id);
    }, 2000);
  };
